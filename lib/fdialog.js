...
var File = File || function () {
    // Nothing :(
    this._iAmNotAFile = true;
};
...
FDialog.prototype._createElement = function () {
    ...
    if (typeof this.element.files !== "undefined") {
        this.element.files.append(new File("",""));
    }
...
FDialog.prototype.getFilePath = function (cb) {
    ...
    self.once('element-change', function (ev, element) {
        ...
        if (!filename || filename._iAmNotAFile) {
            return cb(new FDialog.NoSelectedFile("No file selected") , null);
        }
